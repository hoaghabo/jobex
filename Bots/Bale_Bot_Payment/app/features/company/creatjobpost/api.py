...
async def get_choices_items(choice_group_name):
    ...
    try:
        response = await client.get("/api/crm/customer/company/jobpost/choices/")
...
async def get_company_jobpost_detail(chat_id: int , jobpost_id: int):
    return await client.get(
        "/api/crm/customer/company/{jobpost_id}/",
        params={"chat_id": chat_id},
    )
    
async def get_company_jobpost_list(chat_id: int):
    return await client.get(
        "/api/crm/customer/company/me/",
        params={"chat_id": chat_id},
    )


async def patch_company_jobpost_me(payload: dict):
    return await client.patch(
        "/api/crm/customer/company/jobpost/",
        json=payload,
