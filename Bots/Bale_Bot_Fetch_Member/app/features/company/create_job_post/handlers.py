...
@router.message(F.text == "درخواست آگهی شغلی")
async def choose_jobseeker_handler(message: Message, state: FSMContext):
    ...
    keyboard, company_count = await get_my_company_list_inline_keyboard(message.chat.id)
    await message.answer(
        "لطفا انتخاب کنید برای چه شرکتی قصد درخواست آگهی شغلی دارید؟",
        reply_markup= keyboard
    )
    
    await state.set_state(CompanyJobPostCreateStates.waiting_for_choose_company)



@router.callback_query(
    CompanyJobPostCreateStates.waiting_for_choose_company,
...
async def benefits_handler(message: Message, state: FSMContext):
    ...
    try:
        await create_company_jobpost(chat_id=message.from_user.id , payload=payload)
    except Exception as e:
    ...
    summary_text = (
        ...
        "خلاصه اطلاعات:\n\n"
        f"نام شرکت: {data.get('company_label')}\n"
